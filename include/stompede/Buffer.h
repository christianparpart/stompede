...
/* <stompede/Buffer.h>
 *
 * This file is part of the stompede web server project and is released under LGPL-3.
 * http://www.xzero.io/
 *
...
// }}}
// {{{ free functions API
STOMPEDE_API Buffer& operator<<(Buffer& b, Buffer::value_type v);
STOMPEDE_API Buffer& operator<<(Buffer& b, int v);
STOMPEDE_API Buffer& operator<<(Buffer& b, long v);
STOMPEDE_API Buffer& operator<<(Buffer& b, long long v);
STOMPEDE_API Buffer& operator<<(Buffer& b, unsigned v);
STOMPEDE_API Buffer& operator<<(Buffer& b, unsigned long v);
STOMPEDE_API Buffer& operator<<(Buffer& b, unsigned long long v);
STOMPEDE_API Buffer& operator<<(Buffer& b, const Buffer::value_type *v);
STOMPEDE_API Buffer& operator<<(Buffer& b, const Buffer& v);
STOMPEDE_API Buffer& operator<<(Buffer& b, const BufferRef& v);
STOMPEDE_API Buffer& operator<<(Buffer& b, const std::string& v);
template<typename PodType, size_t N> STOMPEDE_API Buffer& operator<<(Buffer& b, PodType (&v)[N]);
// }}}
...
namespace std {
	template<>
	struct hash<stompede::BufferRef> {
		typedef stompede::BufferRef argument_type;
		typedef uint32_t result_type;

		result_type operator()(const argument_type& value) const noexcept {
			return stompede::hash(value);
		}
	};

	template<>
	struct hash<stompede::Buffer> {
		typedef stompede::Buffer argument_type;
		typedef uint32_t result_type;
